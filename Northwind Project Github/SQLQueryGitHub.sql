--1. This query Display the employee who placed the highest number of orders in February and March, along with the count of their orders:
...
--2. This query Display the last 5 rows from the Employees table:
...
--3. This query Display the product name, price, and units in stock for products that have more units in stock than the maximum amount of any product in category 5:
...
--4. This query Display the category names and total price for categories, along with the average total price of all categories:
with sales as (
...
--5. This query Display the categories where the average product price is higher than the overall average, including the maximum product price in those categories:
...
--6. This query counting the number of products priced above the average product price:
select COUNT(case when UnitPrice > ( select AVG(UnitPrice) from Products) then UnitPrice else NULL end) from Products


--7. This query Display the full names of employees hired after the first 3 employees: 
...
--8. This query Display the category names that have more products than the number of employees:
...
--9. This query Display the number of orders per year:
...
--10. Report that display the number of orders per customer per city, per customer per country, and overall:
...
--11. This query Display the category name, product name, product price, previous product price, and next product price in descending order:
...
--12. Report that display the product number, order number, and service rating based on the time between order date and shipped date:
...
--13. Report that display the product name, order number, revenue from the product, and day of the week of the order:
...
--14. Report that display the number of orders for customer 'SAVEA' in 1996 and 1997, and for customer 'ERNSH' in 1996 and 1997:
...
--15. Report that display the top 5 customers with the highest number of orders, including customer number, company name, order count, and customer ranking:
...
--16. This query display the next details:
...
--17. This Query display the top 3 customers with the lowest order counts, where the customer names start with 'A' or 'L':
...
--18. This Display the customers whose freight cost is above the average, sorted by contact name:
...
--19. Report that detailing the city, company name, and contact name of customers from cities starting with 'A' or 'B' who have placed orders worth more than $1000 in total:
...
--20. Report that listing all even OrderID values where the order's total price is above the median total price of all orders:
...
--21. This query lists employees who do not have any supervisors, along with the number of employees they directly supervise.
select  concat(FirstName,' ',LastName) ,count(employeeid) from Employees where ReportsTo is null group by FirstName,LastName

--22. Report that displaying ContactName, ContactTitle, and CompanyName of customers whose ContactTitle does not include the word "Sales" and who have placed at least one order in the last 6 months.
...
--23. This query displays the CompanyName and ContactName for all customers who do not have a fax number and have made a purchase in the last year. 
